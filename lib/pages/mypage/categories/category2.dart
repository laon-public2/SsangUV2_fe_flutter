...
class _Category1State extends State<Category2> {
  ...
  Future<bool>_loadingProduct() async {
    ...
    await Provider.of<MyPageProvider>(context, listen: false)
        .getProWantCa2(userIdx, page, category);
    await Provider.of<MyPageProvider>(context, listen: false)
        .getProRentCa2(userIdx, page, category);
    return true;
  }
          ...
          builder: (_, _myActHistory, __) {
            return ListView.separated(
              itemCount: _currentItem == '빌려드려요'
                  ? _myActHistory.proRentCa2.length
                  : _myActHistory.proWantCa2.length,
              shrinkWrap: true,
              ...
              itemBuilder: (context, idx) {
                if (_currentItem == "빌려드려요") {
                  return LendItemMyAct(
                    category: '여행',
                    title: '${_myActHistory.proRentCa2[idx].title}',
                    name: _myActHistory.proRentCa2[idx].name,
                    price: _moneyFormat("${_myActHistory.proRentCa2[idx].price}"),
                    status: _myActHistory.proRentCa2[idx].status,
                    idx: _myActHistory.proRentCa2[idx].id,
                    picFile: _myActHistory.proRentCa2[idx].productFiles[0].path,
                    arrayNum: idx,
                    token: _myInfo.accessToken,
                  );
                } else if (_currentItem == '빌려주세요') {
                  return WantItemMyAct(
                    idx: _myActHistory.proWantCa2[idx].id,
                    category:
                    "여행",
                    title: "${_myActHistory.proWantCa2[idx].title}",
                    name: "${_myActHistory.proWantCa2[idx].name}",
                    minPrice: "${_moneyFormat("${_myActHistory.proWantCa2[idx].minPrice}")}원",
                    maxPrice: "${_moneyFormat("${_myActHistory.proWantCa2[idx].maxPrice}")}원",
                    startDate: _dateFormat(_myActHistory.proWantCa2[idx].startDate),
                    endDate: _dateFormat(_myActHistory.proWantCa2[idx].endDate),
                    picture: _myActHistory.proWantCa2[idx].productFiles[0].path,
                  );
                } else {
                  Navigator.push(
                    context,
