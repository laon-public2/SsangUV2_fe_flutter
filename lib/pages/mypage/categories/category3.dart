...
class _Category1State extends State<Category3> {
  ...
  Future<bool>_loadingProduct() async {
    ...
    await Provider.of<MyPageProvider>(context, listen: false)
        .getProWantCa3(userIdx, page, category);
    await Provider.of<MyPageProvider>(context, listen: false)
        .getProRentCa3(userIdx, page, category);
    return true;
  }
          ...
          builder: (_, _myActHistory, __) {
            return ListView.separated(
              itemCount: _currentItem == '빌려드려요'
                  ? _myActHistory.proRentCa3.length
                  : _myActHistory.proWantCa3.length,
              shrinkWrap: true,
              ...
              itemBuilder: (context, idx) {
                if (_currentItem == "빌려드려요") {
                  return LendItemMyAct(
                    category: '스포츠/레저',
                    title: '${_myActHistory.proRentCa3[idx].title}',
                    name: _myActHistory.proRentCa3[idx].name,
                    price: _moneyFormat("${_myActHistory.proRentCa3[idx].price}"),
                    status: _myActHistory.proRentCa3[idx].status,
                    idx: _myActHistory.proRentCa3[idx].id,
                    picFile: _myActHistory.proRentCa3[idx].productFiles[0].path,
                    arrayNum: idx,
                    token: _myInfo.accessToken!,
                  );
                } else if (_currentItem == '빌려주세요') {
                  return WantItemMyAct(
                    idx: _myActHistory.proWantCa3[idx].id,
                    category:
                    "스포츠/레저",
                    title: "${_myActHistory.proWantCa3[idx].title}",
                    name: "${_myActHistory.proWantCa3[idx].name}",
                    minPrice: "${_moneyFormat("${_myActHistory.proWantCa3[idx].minPrice}")}원",
                    maxPrice: "${_moneyFormat("${_myActHistory.proWantCa3[idx].maxPrice}")}원",
                    startDate: _dateFormat(_myActHistory.proWantCa3[idx].startDate),
                    endDate: _dateFormat(_myActHistory.proWantCa3[idx].endDate),
                    picture: _myActHistory.proWantCa3[idx].productFiles[0].path,
                  );
                } else {
                  Navigator.push(
                    context,
