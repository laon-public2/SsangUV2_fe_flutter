              ...
              itemBuilder: (context, idx) {
                if (_currentItem == "빌려드려요") {
                  return LendItemMyAct(
                    ...
                    idx: _myActHistory.proRentCa3[idx].idx,
                    picFile: _myActHistory.proRentCa3[idx].image[0].file,
                    arrayNum: idx,
                    token: _myInfo.accessToken.value,
                  );
                } else if (_currentItem == '빌려주세요') {
                  return WantItemMyAct(
                    idx: _myActHistory.proWantCa3[idx].idx,
                    category:
                    "스포츠/레저",
                    title: "${_myActHistory.proWantCa3[idx].title}",
                    name: "${_myActHistory.proWantCa3[idx].name}",
                    minPrice: "${_moneyFormat("${_myActHistory.proWantCa3[idx].min_price}")}원",
                    maxPrice: "${_moneyFormat("${_myActHistory.proWantCa3[idx].max_price}")}원",
                    startDate: _dateFormat(_myActHistory.proWantCa3[idx].start_date),
                    endDate: _dateFormat(_myActHistory.proWantCa3[idx].end_date),
                    picture: _myActHistory.proWantCa3[idx].image[0].file,
                  );
                } else {
                  Navigator.push(
                    context,
