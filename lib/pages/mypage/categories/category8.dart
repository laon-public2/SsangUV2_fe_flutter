...
UserController userController = Get.find<UserController>();
  ...
  Future<bool>_loadingProduct() async {
    ...
    await myPageController
        .getProWantCa8(userIdx, page, category);
    await myPageController
        .getProRentCa8(userIdx, page, category);
    return true;
  }
          ...
          builder: (_myActHistory) {
            return ListView.separated(
              itemCount: _currentItem == '빌려드려요'
                  ? _myActHistory.proRentCa8.length
                  : _myActHistory.proWantCa8.length,
              shrinkWrap: true,
              ...
              itemBuilder: (context, idx) {
                if (_currentItem == "빌려드려요") {
                  return LendItemMyAct(
                    category: '가구/인테리어',
                    title: '${_myActHistory.proRentCa8[idx].title}',
                    name: _myActHistory.proRentCa8[idx].name,
                    price: _moneyFormat("${_myActHistory.proRentCa8[idx].price}"),
                    status: _myActHistory.proRentCa8[idx].status,
                    idx: _myActHistory.proRentCa8[idx].idx,
                    picFile: _myActHistory.proRentCa8[idx].image[0].file,
                    arrayNum: idx,
                    token: _myInfo.accessToken.value,
                  );
                } else if (_currentItem == '빌려주세요') {
                  return WantItemMyAct(
                    idx: _myActHistory.proWant[idx].idx,
                    category:
                    "가구/인테리어",
                    title: "${_myActHistory.proWantCa8[idx].title}",
                    name: "${_myActHistory.proWantCa8[idx].name}",
                    minPrice: "${_moneyFormat("${_myActHistory.proWantCa8[idx].min_price}")}원",
                    maxPrice: "${_moneyFormat("${_myActHistory.proWantCa8[idx].max_price}")}원",
                    startDate: _dateFormat(_myActHistory.proWantCa8[idx].start_date),
                    endDate: _dateFormat(_myActHistory.proWantCa8[idx].end_date),
                    picture: _myActHistory.proWantCa8[idx].image[0].file,
                  );
                } else {
                  Navigator.push(
                    context,
