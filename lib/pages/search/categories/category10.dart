  ...
  categoryScrollerListener() async {
    ...
    if(categoryScroller.position.pixels == categoryScroller.position.maxScrollExtent){
      ...
      if(_currentItem == "빌려드려요"){
        print("빌려드려요 상태");
        if(pvm.searchPagingCa10.totalCount != pvm.searchDataProductCa10.length){
          print("검색 빌려드려요 more");
          this.page++;
        ...
        print("빌려주세요 상태");
        if(pvm.searchPagingCa10.totalCount != pvm.searchDataProductWantCa10.length){
          this.page++;
          await pvm.SearchingDataProduct(
      ...
      builder: (_, _myList, __) {
        return ListView.separated(
          itemCount: this._currentItem == '빌려드려요'
              ? _myList.searchDataProductCa10.length
              : _myList.searchDataProductWantCa10.length,
          shrinkWrap: true,
          physics: NeverScrollableScrollPhysics(),
          itemBuilder: (context, idx) {
            if (this._currentItem == "빌려드려요") {
              return LendItemMainPage(
                category: "${_category(_myList.searchDataProductCa10[idx].category)}",
                idx: _myList.searchDataProductCa10[idx].id,
                title: "${_myList.searchDataProductCa10[idx].title}",
                name: "${_myList.searchDataProductCa10[idx].name}",
                price: "${_moneyFormat("${_myList.searchDataProductCa10[idx].price}")}원",
                distance:
                "${(_myList.searchDataProductCa10[idx].distance).toStringAsFixed(2)}",
                picture: "${_myList.searchDataProductCa10[idx].productFiles[0].path}",
              );
            } else {
              return WantItemMainPage(
                idx: _myList.searchDataProductWantCa10[idx].id,
                category:
                "${_category(_myList.searchDataProductWantCa10[idx].category)}",
                title: "${_myList.searchDataProductWantCa10[idx].title}",
                name: "${_myList.searchDataProductWantCa10[idx].name}",
                minPrice:
                "${_moneyFormat("${_myList.searchDataProductWantCa10[idx].minPrice}")}원",
                maxPrice:
                "${_moneyFormat("${_myList.searchDataProductWantCa10[idx].maxPrice}")}원",
                distance:
                "${(_myList.searchDataProductWantCa10[idx].distance).toStringAsFixed(2)}",
                startDate: _dateFormat(_myList.searchDataProductWantCa10[idx].startDate),
                endDate: _dateFormat(_myList.searchDataProductWantCa10[idx].endDate),
                picture: _myList.searchDataProductWantCa10[idx].productFiles[0].path,
              );
            }
          },
