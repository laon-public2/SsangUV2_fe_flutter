      ...
      builder: (_, _myList, __) {
        return ListView.separated(
          itemCount: this._currentItem == '빌려드려요'
              ? _myList.searchDataProductCa8.length
              : _myList.searchDataProductWantCa8.length,
          shrinkWrap: true,
          physics: NeverScrollableScrollPhysics(),
          itemBuilder: (context, idx) {
            if (this._currentItem == "빌려드려요") {
              return LendItemMainPage(
                category: "${_category(_myList.searchDataProductCa8[idx].category)}",
                idx: _myList.searchDataProductCa8[idx].id,
                title: "${_myList.searchDataProductCa8[idx].title}",
                name: "${_myList.searchDataProductCa8[idx].name}",
                price: "${_moneyFormat("${_myList.searchDataProductCa8[idx].price}")}원",
                distance:
                "${(_myList.searchDataProduct[idx].distance).toStringAsFixed(2)}",
                picture: "${_myList.searchDataProductCa8[idx].productFiles[0].path}",
              );
            } else {
              return WantItemMainPage(
                idx: _myList.searchDataProductWantCa8[idx].id,
                category:
                "${_category(_myList.searchDataProductWantCa8[idx].category)}",
                title: "${_myList.searchDataProductWantCa8[idx].title}",
                name: "${_myList.searchDataProductWantCa8[idx].name}",
                minPrice:
                "${_moneyFormat("${_myList.searchDataProductWantCa8[idx].minPrice}")}원",
                maxPrice:
                "${_moneyFormat("${_myList.searchDataProductWantCa8[idx].maxPrice}")}원",
                distance:
                "${(_myList.searchDataProductWantCa8[idx].distance).toStringAsFixed(2)}",
                startDate: _dateFormat(_myList.searchDataProductWantCa8[idx].startDate),
                endDate: _dateFormat(_myList.searchDataProductWantCa8[idx].endDate),
                picture: _myList.searchDataProductWantCa8[idx].productFiles[0].path,
              );
            }
          },
