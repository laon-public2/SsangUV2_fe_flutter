  ...
  categoryScrollerListener() async {
    ...
    if(categoryScroller.position.pixels == categoryScroller.position.maxScrollExtent){
      ...
      if(_currentItem == "빌려드려요"){
        print("빌려드려요 상태");
        if(pvm.searchPagingCa8.totalCount != pvm.searchDataProductCa8.length){
          print("검색 빌려드려요 more");
          this.page++;
        ...
        print("빌려주세요 상태");
        if(pvm.searchPagingCa8.totalCount != pvm.searchDataProductWantCa8.length){
          this.page++;
          await pvm.SearchingDataProduct(
  ...
  _toItem() {
        ...
        return ListView.separated(
          itemCount: this._currentItem == '빌려드려요'
              ? productController.searchDataProductCa8.length
              : productController.searchDataProductWantCa8.length,
          shrinkWrap: true,
          physics: NeverScrollableScrollPhysics(),
          itemBuilder: (context, idx) {
            if (this._currentItem == "빌려드려요") {
              return LendItemMainPage(
                category: "${_category(productController.searchDataProductCa8[idx].category)}",
                idx: productController.searchDataProductCa8[idx].id,
                title: "${productController.searchDataProductCa8[idx].title}",
                name: "${productController.searchDataProductCa8[idx].name}",
                price: "${_moneyFormat("${productController.searchDataProductCa8[idx].price}")}원",
                distance:
                "${(productController.searchDataProduct[idx].distance).toStringAsFixed(2)}",
                picture: "${productController.searchDataProductCa8[idx].productFiles[0].path}",
              );
            } else {
              return WantItemMainPage(
                idx: productController.searchDataProductWantCa8[idx].id,
                category:
                "${_category(productController.searchDataProductWantCa8[idx].category)}",
                title: "${productController.searchDataProductWantCa8[idx].title}",
                name: "${productController.searchDataProductWantCa8[idx].name}",
                minPrice:
                "${_moneyFormat("${productController.searchDataProductWantCa8[idx].minPrice}")}원",
                maxPrice:
                "${_moneyFormat("${productController.searchDataProductWantCa8[idx].maxPrice}")}원",
                distance:
                "${(productController.searchDataProductWantCa8[idx].distance).toStringAsFixed(2)}",
                startDate: _dateFormat(productController.searchDataProductWantCa8[idx].startDate),
                endDate: _dateFormat(productController.searchDataProductWantCa8[idx].endDate),
                picture: productController.searchDataProductWantCa8[idx].productFiles[0].path,
              );
            }
          },
