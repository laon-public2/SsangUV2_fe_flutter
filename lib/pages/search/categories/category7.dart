      ...
      builder: (_, _myList, __) {
        return ListView.separated(
          itemCount: this._currentItem == '빌려드려요'
              ? _myList.searchDataProductCa7.length
              : _myList.searchDataProductWantCa7.length,
          shrinkWrap: true,
          physics: NeverScrollableScrollPhysics(),
          itemBuilder: (context, idx) {
            if (this._currentItem == "빌려드려요") {
              return LendItemMainPage(
                category: "${_category(_myList.searchDataProductCa7[idx].category)}",
                idx: _myList.searchDataProductCa7[idx].id,
                title: "${_myList.searchDataProductCa7[idx].title}",
                name: "${_myList.searchDataProductCa7[idx].name}",
                price: "${_moneyFormat("${_myList.searchDataProductCa7[idx].price}")}원",
                distance:
                "${(_myList.searchDataProductCa7[idx].distance).toStringAsFixed(2)}",
                picture: "${_myList.searchDataProductCa7[idx].productFiles[0].path}",
              );
            } else {
              return WantItemMainPage(
                idx: _myList.searchDataProductWantCa7[idx].id,
                category:
                "${_category(_myList.searchDataProductWantCa7[idx].category)}",
                title: "${_myList.searchDataProductWantCa7[idx].title}",
                name: "${_myList.searchDataProductWantCa7[idx].name}",
                minPrice:
                "${_moneyFormat("${_myList.searchDataProductWantCa7[idx].minPrice}")}원",
                maxPrice:
                "${_moneyFormat("${_myList.searchDataProductWantCa7[idx].maxPrice}")}원",
                distance:
                "${(_myList.searchDataProductWantCa7[idx].distance).toStringAsFixed(2)}",
                startDate: _dateFormat(_myList.searchDataProductWantCa7[idx].startDate),
                endDate: _dateFormat(_myList.searchDataProductWantCa7[idx].endDate),
                picture: _myList.searchDataProductWantCa7[idx].productFiles[0].path,
              );
            }
          },
