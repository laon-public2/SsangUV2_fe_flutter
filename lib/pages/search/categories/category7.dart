  ...
  categoryScrollerListener() async {
    ...
    if(categoryScroller.position.pixels == categoryScroller.position.maxScrollExtent){
      ...
      if(_currentItem == "빌려드려요"){
        print("빌려드려요 상태");
        if(pvm.searchPagingCa7.totalCount != pvm.searchDataProductCa7.length){
          print("검색 빌려드려요 more");
          this.page++;
        ...
        print("빌려주세요 상태");
        if(pvm.searchPagingCa7.totalCount != pvm.searchDataProductWantCa7.length){
          this.page++;
          await pvm.SearchingDataProduct(
  ...
  _toItem() {
        ...
        return ListView.separated(
          itemCount: this._currentItem == '빌려드려요'
              ? productController.searchDataProductCa7.length
              : productController.searchDataProductWantCa7.length,
          shrinkWrap: true,
          physics: NeverScrollableScrollPhysics(),
          itemBuilder: (context, idx) {
            if (this._currentItem == "빌려드려요") {
              return LendItemMainPage(
                category: "${_category(productController.searchDataProductCa7[idx].category)}",
                idx: productController.searchDataProductCa7[idx].id,
                title: "${productController.searchDataProductCa7[idx].title}",
                name: "${productController.searchDataProductCa7[idx].name}",
                price: "${_moneyFormat("${productController.searchDataProductCa7[idx].price}")}원",
                distance:
                "${(productController.searchDataProductCa7[idx].distance).toStringAsFixed(2)}",
                picture: "${productController.searchDataProductCa7[idx].productFiles[0].path}",
              );
            } else {
              return WantItemMainPage(
                idx: productController.searchDataProductWantCa7[idx].id,
                category:
                "${_category(productController.searchDataProductWantCa7[idx].category)}",
                title: "${productController.searchDataProductWantCa7[idx].title}",
                name: "${productController.searchDataProductWantCa7[idx].name}",
                minPrice:
                "${_moneyFormat("${productController.searchDataProductWantCa7[idx].minPrice}")}원",
                maxPrice:
                "${_moneyFormat("${productController.searchDataProductWantCa7[idx].maxPrice}")}원",
                distance:
                "${(productController.searchDataProductWantCa7[idx].distance).toStringAsFixed(2)}",
                startDate: _dateFormat(productController.searchDataProductWantCa7[idx].startDate),
                endDate: _dateFormat(productController.searchDataProductWantCa7[idx].endDate),
                picture: productController.searchDataProductWantCa7[idx].productFiles[0].path,
              );
            }
          },
