      ...
      builder: (_, _myList, __) {
        return ListView.separated(
          itemCount: this._currentItem == '빌려드려요'
              ? _myList.searchDataProductCa5.length
              : _myList.searchDataProductWantCa5.length,
          shrinkWrap: true,
          physics: NeverScrollableScrollPhysics(),
          itemBuilder: (context, idx) {
            if (this._currentItem == "빌려드려요") {
              return LendItemMainPage(
                category: "${_category(_myList.searchDataProductCa5[idx].category)}",
                idx: _myList.searchDataProductCa5[idx].id,
                title: "${_myList.searchDataProductCa5[idx].title}",
                name: "${_myList.searchDataProductCa5[idx].name}",
                price: "${_moneyFormat("${_myList.searchDataProductCa5[idx].price}")}원",
                distance:
                "${(_myList.searchDataProductCa5[idx].distance).toStringAsFixed(2)}",
                picture: "${_myList.searchDataProductCa5[idx].productFiles[0].path}",
              );
            } else {
              return WantItemMainPage(
                idx: _myList.searchDataProductWantCa5[idx].id,
                category:
                "${_category(_myList.searchDataProductWantCa5[idx].category)}",
                title: "${_myList.searchDataProductWantCa5[idx].title}",
                name: "${_myList.searchDataProductWantCa5[idx].name}",
                minPrice:
                "${_moneyFormat("${_myList.searchDataProductWantCa5[idx].minPrice}")}원",
                maxPrice:
                "${_moneyFormat("${_myList.searchDataProductWantCa5[idx].maxPrice}")}원",
                distance:
                "${(_myList.searchDataProductWantCa5[idx].distance).toStringAsFixed(2)}",
                startDate: _dateFormat(_myList.searchDataProductWantCa5[idx].startDate),
                endDate: _dateFormat(_myList.searchDataProductWantCa5[idx].endDate),
                picture: _myList.searchDataProductWantCa5[idx].productFiles[0].path,
              );
            }
          },
