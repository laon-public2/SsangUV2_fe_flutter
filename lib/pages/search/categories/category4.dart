      ...
      builder: (_, _myList, __) {
        return ListView.separated(
          itemCount: this._currentItem == '빌려드려요'
              ? _myList.searchDataProductCa4.length
              : _myList.searchDataProductWantCa4.length,
          shrinkWrap: true,
          physics: NeverScrollableScrollPhysics(),
          itemBuilder: (context, idx) {
            if (this._currentItem == "빌려드려요") {
              return LendItemMainPage(
                category: "${_category(_myList.searchDataProductCa4[idx].category)}",
                idx: _myList.searchDataProductCa4[idx].id,
                title: "${_myList.searchDataProductCa4[idx].title}",
                name: "${_myList.searchDataProductCa4[idx].name}",
                price: "${_moneyFormat("${_myList.searchDataProductCa4[idx].price}")}원",
                distance:
                "${(_myList.searchDataProductCa4[idx].distance).toStringAsFixed(2)}",
                picture: "${_myList.searchDataProductCa4[idx].productFiles[0].path}",
              );
            } else {
              return WantItemMainPage(
                idx: _myList.searchDataProductWantCa4[idx].id,
                category:
                "${_category(_myList.searchDataProductWantCa4[idx].category)}",
                title: "${_myList.searchDataProductWantCa4[idx].title}",
                name: "${_myList.searchDataProductWantCa4[idx].name}",
                minPrice:
                "${_moneyFormat("${_myList.searchDataProductWantCa4[idx].minPrice}")}원",
                maxPrice:
                "${_moneyFormat("${_myList.searchDataProductWantCa4[idx].maxPrice}")}원",
                distance:
                "${(_myList.searchDataProductWantCa4[idx].distance).toStringAsFixed(2)}",
                startDate: _dateFormat(_myList.searchDataProductWantCa4[idx].startDate),
                endDate: _dateFormat(_myList.searchDataProductWantCa4[idx].endDate),
                picture: _myList.searchDataProductWantCa4[idx].productFiles[0].path,
              );
            }
          },
