      ...
      builder: (_, _myList, __) {
        return ListView.separated(
          itemCount: this._currentItem == '빌려드려요'
              ? _myList.searchDataProductCa9.length
              : _myList.searchDataProductWantCa9.length,
          shrinkWrap: true,
          physics: NeverScrollableScrollPhysics(),
          itemBuilder: (context, idx) {
            if (this._currentItem == "빌려드려요") {
              return LendItemMainPage(
                category: "${_category(_myList.searchDataProductCa9[idx].category)}",
                idx: _myList.searchDataProductCa9[idx].id,
                title: "${_myList.searchDataProductCa9[idx].title}",
                name: "${_myList.searchDataProductCa9[idx].name}",
                price: "${_moneyFormat("${_myList.searchDataProductCa9[idx].price}")}원",
                distance:
                "${(_myList.searchDataProductCa9[idx].distance).toStringAsFixed(2)}",
                picture: "${_myList.searchDataProductCa9[idx].productFiles[0].path}",
              );
            } else {
              return WantItemMainPage(
                idx: _myList.searchDataProductWantCa9[idx].id,
                category:
                "${_category(_myList.searchDataProductWantCa9[idx].category)}",
                title: "${_myList.searchDataProductWantCa9[idx].title}",
                name: "${_myList.searchDataProductWantCa9[idx].name}",
                minPrice:
                "${_moneyFormat("${_myList.searchDataProductWantCa9[idx].minPrice}")}원",
                maxPrice:
                "${_moneyFormat("${_myList.searchDataProductWantCa9[idx].maxPrice}")}원",
                distance:
                "${(_myList.searchDataProductWantCa9[idx].distance).toStringAsFixed(2)}",
                startDate: _dateFormat(_myList.searchDataProductWantCa9[idx].startDate),
                endDate: _dateFormat(_myList.searchDataProductWantCa9[idx].endDate),
                picture: _myList.searchDataProductWantCa9[idx].productFiles[0].path,
              );
            }
          },
